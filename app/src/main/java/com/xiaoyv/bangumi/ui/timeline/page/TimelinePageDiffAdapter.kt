...
class TimelinePageDiffAdapter(
    ...
    private fun onBindGrid(binding: FragmentTimelinePageGridBinding, item: TimelineEntity) {
    ...
    private fun onBindText(binding: FragmentTimelinePageTextBinding, item: TimelineEntity) {
        binding.ivAvatar.isInvisible = item.avatar.isBlank()
        binding.ivAvatar.loadImageAnimate(item.avatar)
        binding.tvAction.text = item.title
        binding.tvContent.text = item.content
