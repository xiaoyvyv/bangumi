...
public:
     ...
     * @brief コンストラクタ
     */
    LAppModel(LAppTextureManager *textureManager);
    ...
    const Csm::csmChar *GetModelName();

    int GetMotionCount(const Csm::csmChar *motionId);
     ...
     * @param[in]   onBeganMotionHandler
     * モーション再生開始時に呼び出されるコールバック関数。NULLの場合、呼び出されない。
     * @return
...
private:
    LAppTextureManager *_textureManager; // 保存外部传入的 textureManager
