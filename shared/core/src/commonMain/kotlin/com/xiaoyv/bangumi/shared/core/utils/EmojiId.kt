...
private val blakeEmojis by lazy {
    bangumiBlakeSmilesMap.entries.mapIndexed { index, (smileId, url) ->
        BgmEmoji(number = 10001 + index, url = url, smileId = smileId)
    }.toPersistentList()
}

private val musumeEmojis by lazy {
    bangumiMusumeSmilesMap.entries.mapIndexed { index, (smileId, url) ->
        BgmEmoji(number = 20001 + index, url = url, smileId = smileId)
    }.toPersistentList()
}
...
@Composable
fun rememberBlakeEmojis(): SerializeList<BgmEmoji> = blakeEmojis

@Composable
fun rememberMusumeEmojis(): SerializeList<BgmEmoji> = musumeEmojis
