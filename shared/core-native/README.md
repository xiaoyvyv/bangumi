...
渲染器在编译前通过 [`scripts/generate_shaders.sh`](file:///Users/why/AndroidStudioProjects/bangumi-multiplatform/shared/core-native/scripts/generate_shaders.sh) 前置脚本，扫描解压后 Live2D 官方 SDK 内部的 **36 个 GLSL 着色器源码**，并自动生成打包打入 C++ 头文件 `live2d_shaders.h` 的全局内存字典。如果未检测到 SDK 目录，前置脚本会自动触发 [`scripts/setup_sdk.sh`](file:///Users/why/AndroidStudioProjects/bangumi-multiplatform/shared/core-native/scripts/setup_sdk.sh) 自动下载解压。

### 运行前置脚本生成着色器头文件：
```bash
./shared/core-native/scripts/generate_shaders.sh
```
