...
import com.kyant.backdrop.utils.FilterBlurMode
import com.kyant.backdrop.utils.PlatformCapabilities
import com.kyant.backdrop.utils.applyBlurMaskFilter
...
internal class HighlightNode(
    ...
    private fun DrawScope.configurePaint(highlight: Highlight) {
        ...
        paint.applyBlurMaskFilter(blurRadius, FilterBlurMode.NORMAL)
        if (PlatformCapabilities.hasAdvancedShaderCapability()) {
            paint.shader = with(highlight.style) {
                createShader(shape = shapeProvider.shape)
            }
        }
    }
}
